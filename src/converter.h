 ...
 * @param row_delimiters        The delimiters used to separate rows in the string.
 * @param element_delimiters    The delimiters used to separate elements within each row.
 ...
 * @note - See the natively provided `str2matrix_convert_*` functions for conversions to basic data types.
 * @note - If the `conversion_function` returns NULL, the item is NOT added into the matrix and the parsing continues without warning.
...
/** @brief Conversion function to be used with `str2matrix`. Converts string to... string. Returns NULL if parsing fails. */
void *str2matrix_convert_string(const char *string);

/** 
 * @brief Conversion function to be used with `str2matrix`. Converts string to char. Assumes that the string has length 1.
 *        Returns NULL if the first character of the string is 0.
 */
void *str2matrix_convert_char(const char *string);

/** @brief Conversion function to be used with `str2matrix`. Converts string to integer. Returns NULL if parsing fails. */
void *str2matrix_convert_int(const char *string);

/** @brief Conversion function to be used with `str2matrix`. Converts string to float. Returns NULL if parsing fails. */
void *str2matrix_convert_float(const char *string);

/** @brief Conversion function to be used with `str2matrix`. Converts string to size_t. Returns NULL if parsing fails. */
